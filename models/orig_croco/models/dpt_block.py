...
class DPTOutputAdapter(nn.Module):
    ...
    def match_path_to_layer_spatial_resolution(self, path, layer):
        _, _, h, w = layer.shape
        if path.shape[-2:] == layer.shape[-2:]:
            return path
        else:
            path = F.interpolate(path, size=(h, w), mode='bilinear')
            return path
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], image_size):
        ...
        # path_4 = self.match_path_to_layer_spatial_resolution(path_4, layers[2])
        path_3 = self.scratch.refinenet3(path_4, layers[2])

        # path_3 = self.match_path_to_layer_spatial_resolution(path_3, layers[1])
        path_2 = self.scratch.refinenet2(path_3, layers[1])

        # path_2 = self.match_path_to_layer_spatial_resolution(path_2, layers[0])
