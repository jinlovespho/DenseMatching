...
def run(settings, args=None):
    ...
    # GOCor module
    settings.global_optim_iter = 3
    settings.local_optim_iter = 3
    ...
    # 3. Define model
    global_gocor_arguments = {'optim_iter': settings.global_optim_iter, 'apply_query_loss': True,
                              'reg_kernel_size': 3, 'reg_inter_dim': 16, 'reg_output_dim': 16}

    # for global gocor, we apply L_r only
    local_gocor_arguments = {'optim_iter': settings.local_optim_iter}
    model = glunet_vgg16(global_corr_type='GlobalGOCor', global_gocor_arguments=global_gocor_arguments,
                         normalize='leakyrelu', local_corr_type='LocalGOCor',
                         local_gocor_arguments=local_gocor_arguments, same_local_corr_at_all_levels=True,
                         give_flow_to_refinement_module=False, local_decoder_type='OpticalFlowEstimator',
                         global_decoder_type='CMDTop', use_interp_instead_of_deconv=False)
    # in original GLUNet, we set it to False
