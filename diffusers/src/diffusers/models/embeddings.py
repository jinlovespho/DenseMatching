...
def get_2d_sincos_pos_embed(
    ...
    if isinstance(grid_size, int):  # t
        grid_size = (grid_size, grid_size)  # 192 192
    ...
    grid = torch.meshgrid(grid_w, grid_h, indexing="xy")  # here w goes first
    '''
        grid[0]: [ [0, 0.33, 0.66, 1] 
    '''
    grid = torch.stack(grid, dim=0) # 2 1 192 192
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed_max_size = pos_embed_max_size    # 96
        ...
        self.height, self.width = height // patch_size, width // patch_size # 48(96//2) 48(96//2)
        self.base_size = height // patch_size   # 48
        self.interpolation_scale = interpolation_scale

        # Calculate positional embeddings based on max size or default
        if pos_embed_max_size:
            grid_size = pos_embed_max_size  # 96
        else:
            grid_size = int(num_patches**0.5)
        
        # 768 -> vae -> 96 -> patch_size=2 -> 48 

        if pos_embed_type is None:  # f
            self.pos_embed = None
        elif pos_embed_type == "sincos":    # t 
            pos_embed = get_2d_sincos_pos_embed(                # 36864, 1536
                embed_dim,                                      # 1536
                grid_size,                                      # 192
                base_size=self.base_size,                       # 48
                interpolation_scale=self.interpolation_scale,   # 1
                output_type="pt",
            )
            # breakpoint()
            ...
            pos_embed = get_3d_sincos_pos_embed(                # 2304 1536
                embed_dim,
                spatial_size=self.height,                       # 48
                temporal_size=2,
    ...
    def cropped_pos_embed(self, height, width):
        ...
        height = height // self.patch_size      # 48
        width = width // self.patch_size        # 48
    ...
    def forward(self, latent):
        # breakpoint()
        if self.pos_embed_max_size is not None: # t
            height, width = latent.shape[-2:]   # 96 96 
        else:
            height, width = latent.shape[-2] // self.patch_size, latent.shape[-1] // self.patch_size
        latent = self.proj(latent)  # 2 16 96 96 -> 2 1536 48 48 (patch_size=2)
        if self.flatten:    # t
            latent = latent.flatten(2).transpose(1, 2)  # BCHW -> BNC   # 2 2304 1536
        if self.layer_norm: # f
            latent = self.norm(latent)
        if self.pos_embed is None:
            return latent.to(latent.dtype)
        # Interpolate or crop positional embeddings as needed
        if self.pos_embed_max_size: # t
...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        # breakpoint()
        text_embeds = self.text_proj(text_embeds)   # 2 226 4096

        batch_size, num_frames, channels, height, width = image_embeds.shape

        # INTUTION: b f c h w -> b (f h w) d
        if self.patch_size_t is None:   # t
            image_embeds = image_embeds.reshape(-1, channels, height, width)                        #  b f c h w   -> (b f) c H W
            image_embeds = self.proj(image_embeds)                                                  # (b f) d h w 
            image_embeds = image_embeds.view(batch_size, num_frames, *image_embeds.shape[1:])       # (b f) d h w  -> b f d h w
            image_embeds = image_embeds.flatten(3).transpose(2, 3)                                  #  b f d h w   -> b f d (h w)  -> b f (h w) d 
            image_embeds = image_embeds.flatten(1, 2)                                               #  b f (h w) d -> b (f h w) d
        else:
            p = self.patch_size
