...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    def prepare_latents(
        ...
        # JLP- Encode the input image to latent space
        img1_tensor = img1_tensor.to(device=device, dtype=dtype)    # 1 3 768 768
        img1_latents = self.vae.encode(img1_tensor).latent_dist.sample(generator=generator) # 1 16 96 96 
        img1_latents = img1_latents * self.vae.config.scaling_factor
        
        img2_latents = None
        if img2_tensor is not None:
            img2_tensor = img2_tensor.to(device=device, dtype=dtype)
            img2_latents = self.vae.encode(img2_tensor).latent_dist.sample(generator=generator)
            img2_latents = img2_latents * self.vae.config.scaling_factor

        return latents, img1_latents, img2_latents
    ...
    def __call__(
        ...
        # jlp - prepare my input images
        img1 = img1_info['img1']
        img1_cat = img1_info['img1_cat']
        img1_name = img1_info['img1_name']
        img1_tensor = self.image_processor.preprocess(img1, height, width) # 1 3 1024 1024
        ...
        if img2_info is not None:
            ...
            img2_tensor = self.image_processor.preprocess(img2, height, width) # 1 3 1024 1024
        
        # 4. Prepare latent variables
        num_channels_latents = self.transformer.config.in_channels
        latents, img1_latents, img2_latents = self.prepare_latents(
            img1_tensor,
            img2_tensor,
            batch_size * num_images_per_prompt,
            num_channels_latents,
        ...
        # 5. Prepare timesteps
        scheduler_kwargs = {}
        if self.scheduler.config.get("use_dynamic_shifting", None) and mu is None:  # f
        ...
        if (ip_adapter_image is not None and self.is_ip_adapter_active) or ip_adapter_image_embeds is not None: # f
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print(f'INF STOP STEP: {self.args.inf_stop_step}/{self.args.inf_max_step}')
                print(f'CURRENT TIMESTEP: {t}')
                print(f'OUTPUT FEATURE TYPE: {self.args.output_feat_type}')
                print(f'OUTPUT LAYER: {self.args.output_layer}')
                
                if img2_latents is not None and self.args.is_joint:
                    img_latent_model_input = torch.cat([img1_latents, img2_latents], dim=0) # 2 16 128 128
                else:
                    img_latent_model_input = torch.cat([img1_latents] * 2) if self.do_classifier_free_guidance else img1_latents  # 2 16 128 128
                    
                noise = torch.randn_like(img_latent_model_input)    # 2 16 128 128
                timestep = t.expand(img_latent_model_input.shape[0])    # 2
                latent_model_input = self.scheduler.scale_noise(img_latent_model_input, timestep, noise)    # 2 16 128 128

                # # previous code
                # # expand the latents if we are doing classifier free guidance
                # latent_model_input = torch.cat([latents] * 2) if self.do_classifier_free_guidance else latents  # 2 16 128 128
                # # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
                # timestep = t.expand(latent_model_input.shape[0])


                self.args.now = False
                # if t.item() < self.args.time_stop:
                #     self.args.now = True
                ...
                attn_maps = trans_out[1]['attn_maps']
                queries = trans_out[1]['queries']
                keys = trans_out[1]['keys']
                ...
                if not len(attn_maps) == 0 :
                    ...
                    keys = torch.stack(keys, dim=0) # 24 2637 64 (num_layer, img+text, head_dim)
                
                    text_len = prompt_embeds.shape[1]
                    img_len = keys.shape[1] - text_len
                    
                    keys = keys[:, :img_len]
                    return_layer_idx = self.args.output_layer 
                    
                    feat = keys[return_layer_idx:return_layer_idx+1]

                    return feat
                    
                
                # if t.item() < self.args.time_stop:
                #     return noise_pred[0:1]  # 1 16 128 128
