...
def load_image_pair(ann, load_size, image_path="", output_size=None):
...
"""
def draw_correspondences(points1, points2, ann, img1_info, img2_info, image1_label="", image2_label="", title="", radius1=8, radius2=1, save_path=None, iter=None, args=None):
    """
    ...
    if KPTS_VIS_METHOD==1:
        num_points = len(points1)
        
        breakpoint()
        
        
# {"pair_id": 3240, "filename": "003240-2008_007494-2009_004227:dog", "src_imname": "2008_007494.jpg", "trg_imname": "2009_004227.jpg", 
#  "src_imsize": [500, 489, 3], "trg_imsize": [375, 500, 3], "src_bndbox": [1, 9, 323, 489], "trg_bndbox": [34, 224, 191, 500], 
#  "category": "dog", "src_pose": "Rear", "trg_pose": "Rear", 
#  "src_kps": [[220, 16], [127, 36], [184, 84], [176, 122]], "trg_kps": [[137, 250], [68, 275], [96, 239], [114, 258]], 
#  "kps_ids": ["2", "3", "7", "15"], "mirror": 0, "viewpoint_variation": 0, "scale_variation": 1, "truncation": 1, "occlusion": 0}
        
    elif KPTS_VIS_METHOD==2:
        assert len(points1) == len(points2), f"points lengths are incompatible: {len(points1)} != {len(points2)}."
        num_points = len(points1)

        image1 = img1_info['img1']
        image2 = img2_info['img2']
        
        image1_name = img1_info['img1_name']
        image2_name = img2_info['img2_name']
        ...
        # Adjust points2 coordinates
        points2 = np.array(points2)
        points2[:, 0] = points2[:, 0] * scale_y  # y coordinates
        points2[:, 1] = points2[:, 1] * scale_x  # x coordinates
        ...
        # Save individual images with transparent background
        extent1 = ax1.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
        extent2 = ax2.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
        
        fig.savefig(f'{save_path}/{image1_name}.png', bbox_inches=extent1, transparent=True)
        fig.savefig(f'{save_path}/{image2_name}.png', bbox_inches=extent2, transparent=True)
        
        img_src = cv2.imread(f'{save_path}/{image1_name}.png')
        img_trg = cv2.imread(f'{save_path}/{image2_name}.png')
        
        combined_img = cv2.hconcat([img_src, img_trg])
        
        # Delete individual images
        os.remove(f'{save_path}/{image1_name}.png')
        os.remove(f'{save_path}/{image2_name}.png')
        # Close the figure to free memory
        plt.close(fig)
        
        if args.log_tool == 'wandb':
            # Check logging frequency
            if iter % args.WANDB_LOG_FREQ == 0:
                wandb.log({f"vis_KPTS_{img1_info['img1_cat']}/pred_src{image1_name}_trg{image2_name}": wandb.Image(combined_img[..., ::-1])})
        else:
            cv2.imwrite(f'{save_path}/pred_src{image1_name}_trg{image2_name}.png', combined_img)
