    ...
    def __getitem__(self, idx):
        ...
        if self.two_views:
            ...
            if type(mask) in [tuple, list]:
                # flow field at different resolution
                for i in range(len(flow)):
                    mask[i] = torch.from_numpy(mask[i].astype(bool if version.parse(torch.__version__) >= version.parse("1.1") else np.uint8))
            else:
                mask = torch.from_numpy(mask.astype(bool if version.parse(torch.__version__) >= version.parse("1.1") else np.uint8))
            ...
            if self.compute_mask_zero_borders:
                output['mask_zero_borders'] = mask_valid.astype(bool) if version.parse(torch.__version__) >= version.parse("1.1") \
