...
class MultiscaleBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x: b 13 196 196+128: b L h1w1 h2w2+d
        '''
        # breakpoint()
        B, L, N, D = x.shape
        ...
        x = x.flatten(0, 1) # BL N D
        x = self.block_1(x)
        x = self.block_2(x)
        
        x = x.view(B, L, N, -1).transpose(1, 2).flatten(0, 1) 
        x = x + self.drop_path(self.attn_multiscale(self.norm1(x)))
        x = x.view(B, N, L, -1).transpose(1, 2).flatten(0, 1)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        x = x.view(B, L, N, -1)
...
class TransformerAggregator(nn.Module):
    ...
    def forward(self, attn_maps, feat):
        ...
        # 
        one_enc_feat = feat[:,0:1,...]
        dec_feats = feat[:,1:,...]

        x = torch.cat((x, feat), dim=3) + pos_embed
        x = self.blocks(x)  # b 13 196 196+128
        
        x = self.proj(x) + attn_maps  # b 13 196 196
...
class CATs_SWIN(nn.Module):
    ...
    def forward(self, attn_maps_tgt, decfeats_tgt, output_shape, encfeat_last_src, encfeat_last_tgt, attn_maps_src=None, decfeats_src=None, tgt_img = None, src_img = None):
        ...
        refined_corr = self.decoder(attn_maps_tgt, decfeats_tgt)
        
        if self.args.reverse:   # false
            decfeats_src = torch.stack(src_feats_proj, dim=1)
            attn_maps_src = torch.stack(attn_maps_src, dim=1)
            refined_corr_source = self.decoder(attn_maps_src, decfeats_src)
            refined_corr_target = refined_corr
            refined_corr = (refined_corr_source.transpose(-1,-2))
        elif self.reciprocity:  # true
            decfeats_src = torch.stack(src_feats_proj, dim=1)
            attn_maps_src = torch.stack(attn_maps_src, dim=1)
            refined_corr_source = self.decoder(attn_maps_src, decfeats_src) # b 196 196
            refined_corr_target = refined_corr
            refined_corr = (refined_corr + refined_corr_source.transpose(-1,-2)) / 2    # b 196 196
        ...
        flow = unnormalise_and_convert_mapping_to_flow(flow)    # b 2 14 14
        h, w = flow.shape[-2:]
        if self.output_interp:  # true
            flow = F.interpolate(flow, size=output_shape, mode='bilinear', align_corners=False) # b 2 224 224
