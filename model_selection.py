...
def select_model(model_name, pre_trained_model_type, args, global_optim_iter, local_optim_iter,
    ...
    if model_name == 'GLUNet':
        ...
        '''
        Default for global and local gocor args:
    ...
    # define inference args
    if args.network_type == 'PDCNet' or args.network_type == 'PDCNet_plus':
        # define inference parameters for PDC-Net and particularly the ones needed for multi-stage alignment
        network.set_inference_parameters(confidence_R=args.confidence_map_R,
                                         ransac_thresh=args.ransac_thresh,
                                         multi_stage_type=args.multi_stage_type,
                                         mask_type_for_2_stage_alignment=args.mask_type,
                                         homography_visibility_mask=args.homography_visibility_mask,
                                         list_resizing_ratios=args.scaling_factors,
                                         compute_cyclic_consistency_error=args.compute_cyclic_consistency_error)
