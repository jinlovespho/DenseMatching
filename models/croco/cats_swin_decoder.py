...
class CATs_SWIN_Decoder(nn.Module):
    def __init__(self,
    feature_size=16,
    ...
    args=None,):
        ...
        self.upsample1 = nn.ConvTranspose2d(1024, 256, kernel_size=2, stride=2)
        self.upsample2 = nn.ConvTranspose2d(1024, 128, kernel_size=4, stride=4)
    ...
    def forward(self, attn_maps_tgt, decfeats_tgt, output_shape, encfeat_last_src, encfeat_last_tgt, attn_maps_src=None, encfeats_src=None, tgt_img = None, src_img = None, appearance_feature = None):
        # breakpoint()
        ...
        if self.correlation:    # true
            ...
            attn_maps_tgt = [corr] + attn_maps_tgt
            ...
            if self.reciprocity:    # true
                ...
                encfeats_src = [encfeat_last_src] + encfeats_src
        
        for i in range(len(self.proj)):
            B,L,C = decfeats_tgt[i].shape
            
            decfeats_tgt[i] = self.ln[i](decfeats_tgt[i])
            tgt_feats_proj.append(self.proj[i](decfeats_tgt[i]))
            
            if self.reciprocity:    # true
                encfeats_src[i] = self.ln_src[i](encfeats_src[i])
                src_feats_proj.append(self.proj[i](encfeats_src[i]))
        ...
        refined_corr = self.decoder(attn_maps_tgt, decfeats_tgt)    # b 2 196 196
        
        if self.args.reverse:   # false
            encfeats_src = torch.stack(src_feats_proj, dim=1)
            attn_maps_src = torch.stack(attn_maps_src, dim=1)
            refined_corr_source = self.decoder(attn_maps_src, encfeats_src)
            ...
            refined_corr_source = self.decoder(attn_maps_src, encfeats_src)
            refined_corr_target = refined_corr
            refined_corr = (refined_corr + refined_corr_source.transpose(-1,-2)) / 2    # b 2 196 196
        ...
        coarse_flow = unnormalise_and_convert_mapping_to_flow(coarse_flow)  # b 2 14 14 
        h, w = coarse_flow.shape[-2:]
        if self.output_interp:  # true
        ...
        upsampled_appearance_feature = [self.upsample1(appearance_feature[0]),self.upsample2(appearance_feature[1])]
