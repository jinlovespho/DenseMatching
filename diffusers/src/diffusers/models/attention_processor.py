...
class JointAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        residual = hidden_states    # 2 4096 1536

        batch_size = hidden_states.shape[0] # 2

        # `sample` projections.
        query = attn.to_q(hidden_states)    # 2 4096 1536
        key = attn.to_k(hidden_states)    # 2 4096 1536
        value = attn.to_v(hidden_states)    # 2 4096 1536

        inner_dim = key.shape[-1]   # 1536
        head_dim = inner_dim // attn.heads    # 1536 // 24 = 64

        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)    # 2 24 4096 64
        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)        # 2 24 4096 64
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)    # 2 24 4096 64

        if attn.norm_q is not None: # f
            query = attn.norm_q(query)    
        if attn.norm_k is not None: # f
            key = attn.norm_k(key)

        # `context` projections.
        if encoder_hidden_states is not None: # t
            encoder_hidden_states_query_proj = attn.add_q_proj(encoder_hidden_states)    # 2 333 1536
            encoder_hidden_states_key_proj = attn.add_k_proj(encoder_hidden_states)    # 2 333 1536
            encoder_hidden_states_value_proj = attn.add_v_proj(encoder_hidden_states)   # 2 333 1536

            encoder_hidden_states_query_proj = encoder_hidden_states_query_proj.view(   # 2 24 33 64
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            encoder_hidden_states_key_proj = encoder_hidden_states_key_proj.view(   # 2 24 33 64
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            encoder_hidden_states_value_proj = encoder_hidden_states_value_proj.view(   # 2 24 33 64
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)

            if attn.norm_added_q is not None:   # f
                encoder_hidden_states_query_proj = attn.norm_added_q(encoder_hidden_states_query_proj)
            if attn.norm_added_k is not None:   # f
                encoder_hidden_states_key_proj = attn.norm_added_k(encoder_hidden_states_key_proj)

            # breakpoint()
            query = torch.cat([query, encoder_hidden_states_query_proj], dim=2) # 2 24 4096+333 64  # 2 24 2304+333 64
            key = torch.cat([key, encoder_hidden_states_key_proj], dim=2)   # 2 24 4429 64
            value = torch.cat([value, encoder_hidden_states_value_proj], dim=2)   # 2 24 4429 64

        # breakpoint()
        if my_args.joint_full_attn: 
            b,h,n,d = query.shape   # 2 24 2637 64
            query = rearrange(query, 'b h n d -> 1 h (b n) d')      # 2 24 2637 64 -> 1 24 5274 64
        ...
        if my_args.output_feat_type == 'attn_map':
            ...
            feat_img = feat[:, :img_len]                        # 2 4096 1536
            EXTRACT_FEAT = feat_img
        
        elif my_args.output_feat_type == 'key':
            feat = key.clone().detach().cpu()                 # 2 24 4429 64   
            feat = rearrange(feat, 'b h n d -> b n (h d)')      # 2 4429 1536
            feat_img = feat[:, :img_len]                        # 2 4096 1536
            EXTRACT_FEAT = feat_img
            
        elif my_args.output_feat_type == 'value':
            feat = value.clone().detach().cpu()                 # 2 24 4429 64   
            feat = rearrange(feat, 'b h n d -> b n (h d)')      # 2 4429 1536
            feat_img = feat[:, :img_len]                        # 2 4096 1536
            EXTRACT_FEAT = feat_img

        hidden_states = torch.matmul(attn_scores, value)    # 2 24 2637 64  # joint_attn=True: 1 24 5274 64
        
        if my_args.joint_full_attn: 
            hidden_states = rearrange(hidden_states, '1 h (b n) d -> b h n d', b=b)  # joint_attn=True: 2 24 2637 64
        
        # hidden_states = F.scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False) 
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)    # 2 2637 1536
        hidden_states = hidden_states.to(query.dtype)

        if encoder_hidden_states is not None:   # t
            # Split the attention outputs.
            hidden_states, encoder_hidden_states = (        # hidden_states: 2 2304 1536
                hidden_states[:, : residual.shape[1]],      # encoder_hidden_states: 2 333 1536
                hidden_states[:, residual.shape[1] :],
            )
            if not attn.context_pre_only:   # t
                encoder_hidden_states = attn.to_add_out(encoder_hidden_states)  # 2 333 1536

        # linear proj
        hidden_states = attn.to_out[0](hidden_states)   # 2 2304 1536 
        # dropout
        hidden_states = attn.to_out[1](hidden_states)   # 2 2304 1536
            
        if encoder_hidden_states is not None:
            return hidden_states, encoder_hidden_states, EXTRACT_FEAT
        else:
            return hidden_states
...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)    # 2 1576 1920

        batch_size, sequence_length, _ = (  # 2, 226
            hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
        )
        ...
        query = attn.to_q(hidden_states)    # 2 1576 1920
        key = attn.to_k(hidden_states)
        value = attn.to_v(hidden_states)
        ...
        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)    # 2 30 1576 64 (b head txt_img head_dim)
        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)

        if attn.norm_q is not None: # t
            query = attn.norm_q(query)      # 2 30 1576 64
        if attn.norm_k is not None: # t
            key = attn.norm_k(key)          # 2 30 1576 64

        # Apply RoPE if needed
        if image_rotary_emb is not None:    # f
            from .embeddings import apply_rotary_emb
        ...
        # JLP - extract feature
        self.cogvid_attn = hidden_states[0].clone().detach().cpu()
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        residual = hidden_states    # 2 1024 1152
        if attn.spatial_norm is not None:   # f
            hidden_states = attn.spatial_norm(hidden_states, temb)
        ...
        batch_size, sequence_length, _ = (  # 2, 1024
            hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
        )
        ...
        if attn.group_norm is not None: # f
            hidden_states = attn.group_norm(hidden_states.transpose(1, 2)).transpose(1, 2)

        query = attn.to_q(hidden_states)    # 2 1024 1152

        if encoder_hidden_states is None:   # t
            encoder_hidden_states = hidden_states   # 2 1024 1152
        elif attn.norm_cross:
            encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)

        key = attn.to_k(encoder_hidden_states)    # 2 1024 1152
        value = attn.to_v(encoder_hidden_states)  # 2 1024 1152

        inner_dim = key.shape[-1]
        head_dim = inner_dim // attn.heads

        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)    # 2 16 1024 72

        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)        # 2 16 1024 72
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)    # 2 16 1024 72
        ...
        # if query.shape == (2, 16, 1024, 72):
        #     breakpoint()

        # the output of sdp = (batch, num_heads, seq_len, head_dim)
        # TODO: add support for attn.scale when we move to Torch 2.1
        # hidden_states = F.scaled_dot_product_attention( # 2 16 1024 72 
        #     query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        # )
        
        # JLP - manual attention computation
        attn_scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(head_dim)  # 2 16 1024 1024
        attn_scores = F.softmax(attn_scores, dim=-1)                                    # 2 16 1024 1024
        hidden_states = torch.matmul(attn_scores, value)                                # 2 16 1024 72

        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)    # 2 1024 1152
        hidden_states = hidden_states.to(query.dtype)

        # linear proj
        hidden_states = attn.to_out[0](hidden_states)   # 2 1024 1152
        # dropout
        hidden_states = attn.to_out[1](hidden_states)   # 2 1024 1152
        
        # JLP - save features for extraction
        self.dit_attn = hidden_states[0].clone().detach().cpu()    # 1024 1152

        if input_ndim == 4: # f
            hidden_states = hidden_states.transpose(-1, -2).reshape(batch_size, channel, height, width)

        if attn.residual_connection:    # f
            hidden_states = hidden_states + residual

        hidden_states = hidden_states / attn.rescale_output_factor  # 2 1024 1152, where attn.rescale_output_factor = 1.0
