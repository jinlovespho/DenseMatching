...
def make_scratch(in_shape, out_shape, groups=1, expand=False, args=None):
    scratch = nn.Module()
...
class HierarchicalCATs(nn.Module):
    """DPT output adapter.
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], coarse_flows, image_size):
        ...
        if self.args.cost_agg == 'hierarchical_cats':
            ...
            path_1 = self.scratch.refinenet3(path_2,layers1)
            out1 = self.head3(path_1)
            ...
            path_0 = self.scratch.refinenet2(path_1,layers0)
            out0 = self.head2(path_0)
