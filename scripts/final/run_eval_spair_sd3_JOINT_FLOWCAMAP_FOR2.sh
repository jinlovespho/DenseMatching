        ...
        for inf_step_count in 1; do
            ...
            for msk_attn in -1; do 
                ...
                for vis_layer in "9,10,11,12"; do
                    ...
                    for beta in 1e-4; do
                        # Loop through different attention map heads
                        for head in mean; do
                            # Loop through different attention map filter
                            for filter in try1_reciprocal; do
                                ...
                                LOG_ARGS="
                                    ...
                                    --wandb_exp_name server5_spair_SORTED_VALSPLIT360_sd3_JOINT_FLOWCAMAP_BETA${beta}_HEAD${head}_FILTER${filter}_step_max28_stop${stop_step}_count${inf_step_count}_${feat_type}_MASKATTN${msk_attn}_layer${vis_layer}_gpu${CUDA} \
                                    --save_dir ./vis/spair_SORTED_VALSPLIT360/sd3_JOINT/FLOWCAMAP_BETA${beta}_HEAD${head}_FILTER${filter}_step_max28_stop${stop_step}_count${inf_step_count}_${feat_type}_MASKATTN${msk_attn}_layer${vis_layer} \
                                "
